...
exports.createPaymentOrder = async (req, res) => {
  try {
    ...
    const updateData = { orderId: response?.id };

    if (response.id) {
      updateData['status'] = 'CREATED';
      await PaymentOrder.findByIdAndUpdate(
        paymentOrderDetails._id,
        { status: updateData.status, orderId: updateData.orderId },
        { new: true, runValidators: true }
      );
    } else {
      updateData['status'] = 'FAILED';
...
exports.capture = async (req, res) => {
  try {
    ...
    if (req.body.event === "payment.captured") {
      ...
      await PaymentTransaction.updateOne(
        { orderId: transactionData.orderId, paymentId: transactionData.paymentId },
        { status: "FAILED", description: req.body.payload.payment.entity.error_description }
      );
